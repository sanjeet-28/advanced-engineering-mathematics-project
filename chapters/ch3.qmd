...
The concepts and methods of solving linear ODEs of order $n=2$ extend nicely to linear ODEs of higher order $n$, that is, $n=3, 4$, etc. This shows that the theory explained in <!-- TODO: replace Chap. 2 with actual chapter label --> for second-order linear ODEs is attractive, since it can be extended in a straightforward way to arbitrary $n$. We do so in this chapter and notice that the formulas become more involved, the variety of roots of the characteristic equation (in @sec-homogeneous-linear-odes-constant-coeff) becomes much larger with increasing $n$, and the Wronskian plays a more prominent role.

The concepts and methods of solving second-order linear ODEs extend readily to linear ODEs of higher order.

This chapter follows <!-- TODO: replace Chap. 2 with actual chapter label --> naturally, since the results of <!-- TODO: replace Chap. 2 with actual chapter label --> can be readily extended to that of this chapter.

**Prerequisite:** <!-- TODO: replace Secs. 2.1, 2.2, 2.6, 2.7, 2.10 with actual cross-reference labels -->.
**References and Answers to Problems:** App. 1 Part A, and App. 2.

## Homogeneous Linear ODEs {#sec-homogeneous-linear-odes}

Recall from <!-- TODO: replace Sec. 1.1 with actual cross-reference label --> that an ODE is of $n$th order if the $n$th derivative of the unknown function $y(x)$ is the highest occurring derivative. Thus the ODE is of the form
...
and is called **homogeneous**. If $r(x)$ is not identically zero, then the ODE is called **nonhomogeneous**. This is as in <!-- TODO: replace Sec. 2.1 with actual cross-reference label -->.
...
@sec-homogeneous-linear-odes and @sec-homogeneous-linear-odes-constant-coeff will be devoted to homogeneous linear ODEs and @sec-nonhomogeneous-linear-odes to nonhomogeneous linear ODEs.

### Homogeneous Linear ODE: Superposition Principle, General Solution

The basic superposition or linearity principle of <!-- TODO: replace Sec. 2.1 with actual cross-reference label --> extends to $n$th order homogeneous linear ODEs as follows.
...
The proof is a simple generalization of that in <!-- TODO: replace Sec. 2.1 with actual cross-reference label --> and we leave it to the student.

Our further discussion parallels and extends that for second-order ODEs in <!-- TODO: replace Sec. 2.1 with actual cross-reference label -->. So we next define a general solution of (@eq-3-2), which will require an extension of linear independence from 2 to $n$ functions.
...
Note that when $n = 2$ these concepts reduce to those defined in <!-- TODO: replace Sec. 2.1 with actual cross-reference label -->.
...
**Solution.** As in <!-- TODO: replace Sec. 2.2 with actual cross-reference label --> we substitute $y = e^{\lambda x}$. Omitting the common factor $e^{\lambda x}$ we obtain the characteristic equation
$$
...
In extension of the existence and uniqueness theorem in <!-- TODO: replace Sec. 2.6 with actual cross-reference label --> we now have the following.
...
$$
**Solution.** **Step 1. General solution.** As in <!-- TODO: replace Sec. 2.5 with actual cross-reference label --> we try $y = x^m$. By differentiation and substitution,
$$
x^3m(m-1)(m-2)x^{m-3} - 3x^2m(m-1)x^{m-2} + 6xmx^{m-1} - 6x^m = 0.
$$
Dropping $x^m$ and ordering gives $m^3 - 6m^2 + 11m - 6 = 0$. If we can guess the root $m = 1$, we can divide by $m-1$ and find the other roots 2 and 3, thus obtaining the solutions $x, x^2, x^3$, which are linearly independent on $I$ (see Example 2). [In general one shall need a root-finding method, such as Newton's (<!-- TODO: replace Sec. 19.2 with actual cross-reference label -->), also available in a CAS (Computer Algebra System).] Hence a general solution is
$$
...
(b) $c_1 + 2c_2 + 3c_3 = 1$
(c) $2c_2 + 6c_3 = -4$.
This is solved by Cramer's rule (<!-- TODO: replace Sec. 7.6 with actual cross-reference label -->), or by elimination, which is simple, as follows. (b) - (a) gives
(d) $c_2 + 2c_3 = -1$.
...
Linear independence of solutions is crucial for obtaining general solutions. Although it can often be seen by inspection, it would be good to have a criterion for it. Now Theorem 2 in <!-- TODO: replace Sec. 2.6 with actual cross-reference label --> extends from order $n=2$ to any $n$. This extended criterion uses the **Wronskian** $W$ of $n$ solutions $y_1, \dots, y_n$ defined as the $n$th-order determinant
...
\end{aligned}
$$ {#eq-3-8}
(@eq-3-7), (@eq-3-8) is a homogeneous linear system of algebraic equations with a nontrivial solution $k_1, \dots, k_n$. Hence its coefficient determinant must be zero for every $x$ on $I$, by Cramer's theorem (<!-- TODO: replace Sec. 7.7 with actual cross-reference label -->). But that determinant is the Wronskian $W$, as we see from (@eq-3-6). Hence $W$ is zero for every $x$ on $I$.
...
Let us first show that general solutions always exist. Indeed, Theorem 3 in <!-- TODO: replace Sec. 2.6 with actual cross-reference label --> extends as follows.
...
\end{aligned}
$$ {#eq-3-10}
But this is a linear system of equations in the unknowns $c_1, \dots, c_n$. Its coefficient determinant is the Wronskian $W$ of $y_1, \dots, y_n$ at $x_0$. Since $y_1, \dots, y_n$ form a basis, they are linearly independent, so that $W$ is not zero by Theorem 3. Hence (@eq-3-10) has a unique solution (by Cramer's theorem in <!-- TODO: replace Sec. 7.7 with actual cross-reference label -->). With these values $c_1 = C_1, \dots, c_n = C_n$ we obtain the particular solution
$$
y^*(x) = C_1 y_1(x) + \dots + C_n y_n(x)
$$
...
This completes our theory of the homogeneous linear ODE (@eq-3-2). Note that for $n = 2$ it is identical with that in <!-- TODO: replace Sec. 2.6 with actual cross-reference label -->. This had to be expected.
...
7. **TEAM PROJECT. General Properties of Solutions of Linear ODEs.** These properties are important in obtaining new solutions from given ones. Therefore extend Team Project 38 in <!-- TODO: replace Sec. 2.2 with actual cross-reference label --> to $n$th-order ODEs. Explore statements on sums and multiples of solutions of (@eq-3-1) and (@eq-3-2) systematically and with proofs. Recognize clearly that no new ideas are needed in this extension from $n = 2$ to general $n$.
...
## Homogeneous Linear ODEs with Constant Coefficients {#sec-homogeneous-linear-odes-constant-coeff}

We proceed along the lines of <!-- TODO: replace Sec. 2.2 with actual cross-reference label -->, and generalize the results from $n = 2$ to arbitrary $n$. We want to solve an $n$th-order homogeneous linear ODE with constant coefficients, written as

$$
y^{(n)} + a_{n-1}y^{(n-1)} + \dots + a_1y' + a_0y = 0
$$ {#eq-3-11}

where $y^{(n)} = d^ny/dx^n$, etc. As in <!-- TODO: replace Sec. 2.2 with actual cross-reference label -->, we substitute $y = e^{\lambda x}$ to obtain the characteristic equation

$$
\lambda^n + a_{n-1}\lambda^{n-1} + \dots + a_1\lambda + a_0 = 0
$$ {#eq-3-12}

of (@eq-3-11). If $\lambda$ is a root of (@eq-3-12), then $y = e^{\lambda x}$ is a solution of (@eq-3-11). To find these roots, you may need a numeric method, such as Newton's in <!-- TODO: replace Sec. 19.2 with actual cross-reference label -->, also available on the usual CASs.
...
If complex roots occur, they must occur in conjugate pairs since the coefficients of (@eq-3-11) are real. Thus, if $\lambda = \gamma + i\omega$ is a simple root of (@eq-3-12), so is the conjugate $\bar{\lambda} = \gamma - i\omega$ and two corresponding linearly independent solutions are (as in <!-- TODO: replace Sec. 2.2 with actual cross-reference label -->, except for notation)
...
If a real double root occurs, say, $\lambda_1 = \lambda_2$, then $y_1 = y_2$ in (@eq-3-13), and we take $e^{\lambda_1 x}$ and $xe^{\lambda_1 x}$ as corresponding linearly independent solutions. This is as in <!-- TODO: replace Sec. 2.2 with actual cross-reference label -->.
...
14. **PROJECT. Reduction of Order.** This is of practical interest since a single solution of an ODE can often be guessed. For second order, see Example 7 in <!-- TODO: replace Sec. 2.1 with actual cross-reference label -->.
...
## Nonhomogeneous Linear ODEs {#sec-nonhomogeneous-linear-odes}
...
with $x_0$ in $I$. Under those continuity assumptions it has a unique solution. The ideas of proof are the same as those for $n = 2$ in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->.
...
($a_0, \dots, a_{n-1}$ constant) and special $r(x)$ as in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->, such a $y_p(x)$ can be determined by the method of undetermined coefficients, as in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->, using the following rules.

(A) **Basic Rule** as in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->.
...
(C) **Sum Rule** as in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->.

The practical application of the method is the same as that in <!-- TODO: replace Sec. 2.7 with actual cross-reference label -->. It suffices to illustrate the typical steps of solving an initial value problem and, in particular, the new Modification Rule, which includes the old Modification Rule as a particular case (with $n = 2, k = 1$ or 2). We shall see that the technicalities are the same as for $n = 2$ except perhaps for the more involved determination of the constants.
...
The method of variation of parameters (see <!-- TODO: replace Sec. 2.10 with actual cross-reference label -->) also extends to arbitrary order $n$. It gives a particular solution $y_p$ for the nonhomogeneous equation (@eq-3-23) (in standard form with $y^{(n)}$ as the first term!) by the formula
...
The proof of (@eq-3-29) uses an extension of the idea of the proof of (2) in <!-- TODO: replace Sec. 2.10 with actual cross-reference label --> and can be found in Ref [A11] listed in App. 1.
...
A related application of vibration of beams does not fit in here since it leads to PDEs and will therefore be discussed in <!-- TODO: replace Sec. 12.3 with actual cross-reference label -->.
...
Compare with the similar Summary of <!-- TODO: replace Chap. 2 with actual chapter label --> (the case $n = 2$).

This chapter extends <!-- TODO: replace Chap. 2 with actual chapter label --> from order $n = 2$ to arbitrary order $n$. An $n$th-order linear ODE is an ODE that can be written
...
the superposition principle (@sec-homogeneous-linear-odes) holds, just as in the case $n = 2$. A **basis** or **fundamental system** of solutions of (@eq-3-33) on $I$ consists of $n$ linearly independent solutions $y_1, \dots, y_n$ of (@eq-3-33) on $I$. A **general solution** of (@eq-3-33) on $I$ is a linear combination of these,
...
$$
y = y_h + y_p \quad \text{(@sec-nonhomogeneous-linear-odes)}.
$$ {#eq-3-35}

Here, $y_p$ is a particular solution of (@eq-3-32) and is obtained by two methods (undetermined coefficients or variation of parameters) explained in @sec-nonhomogeneous-linear-odes.

An **initial value problem** for (@eq-3-32) or (@eq-3-33) consists of one of these ODEs and $n$ initial conditions (@sec-homogeneous-linear-odes, @sec-nonhomogeneous-linear-odes)
