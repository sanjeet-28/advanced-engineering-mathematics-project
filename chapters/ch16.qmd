---
title: "Laurent Series. Residue Integration"
---

The main purpose of this chapter is to learn about another powerful method for evaluating complex integrals and certain real integrals. It is called **residue integration**. Recall that the first method of evaluating complex integrals consisted of directly applying Cauchy’s integral formula of @sec-cauchy-integral-formula. Then we learned about Taylor series (<!-- TODO: replace Chap. 15 with actual chapter label -->) and will now generalize Taylor series. The beauty of residue integration, the second method of integration, is that it brings together a lot of the previous material.
...
**Prerequisite:** <!-- TODO: replace Chaps. 13 with actual chapter label -->, 14, <!-- TODO: replace Sec. 15.2 with actual cross-reference label -->.  
**Sections that may be omitted in a shorter course:** 16.2, 16.4.  
**References and Answers to Problems:** App. 1 Part D, App. 2.

## Laurent Series {#sec-laurent-series-sec}
...
Laurent series are also used for classifying singularities (@sec-singularities-zeros) and in a powerful integration method (“residue integration,” @sec-16-3).
...
$$ {#eq-16-1-9}
Compare this for a moment with the expansion of $1/(z^* - z)$ in @sec-taylor-maclaurin-series, to really understand the difference. Then go on and apply formula for a finite geometric sum in @sec-sequences-series-tests, obtaining
$$
...
**Solution.** From the Maclaurin series of $\sin z$ in @sec-taylor-maclaurin-series, we obtain
$$
...
**Solution.** From the Maclaurin series of $e^z$ in @sec-taylor-maclaurin-series with $z$ replaced by $1/z$, we obtain a Laurent series whose principal part is an infinite series:
$$
...
## PROBLEM SET 16.1 {#sec-problem-set-16-1}
...
## Singularities and Zeros. Infinity {#sec-singularities-zeros}
...
**Taylor Series at a Zero.** At an $n$-th order zero of $f(z)$, the derivatives $f(z_0), \dots, f^{(n-1)}(z_0)$ are zero, by definition. Hence the first few coefficients of the Taylor series in @sec-taylor-maclaurin-series are zero, too, whereas $a_n = f^{(n)}(z_0)/n! \neq 0$, so that this series takes the form
$$
...
## PROBLEM SET 16.2 {#sec-problem-set-16-2}
...
## Residue Integration Method {#sec-16-3}

If $f(z)$ is analytic everywhere on a simple closed path $C$ and inside $C$, then by Cauchy's integral theorem (<!-- TODO: replace Sec. 14.2 with actual cross-reference label -->),
$$
\oint_C f(z)\,dz = 0.
$$
...
$$
that converges for all points near $z_0$ (except at $z_0$ itself), in some domain of the form $0 < |z - z_0| < R$ (sometimes called a *deleted neighborhood*, an old-fashioned term that we shall not use). Now comes the key idea. The coefficient $b_1$ of the first negative power $1/(z - z_0)$ of this Laurent series is given by the integral formula (2) in @sec-laurent-series-sec with $n = 1$, namely,
$$
b_1 = \frac{1}{2\pi i} \oint_C f(z)\,dz.
$$

Now, since we can obtain Laurent series by various methods, without using the integral formulas for the coefficients (see the examples in @sec-laurent-series-sec), we can find $b_1$ by one of those methods and then use the formula for $b_1$ for evaluating the integral, that is,
...
**Solution.** From (14) in <!-- TODO: replace Sec. 15.4 with actual cross-reference label --> we obtain the Laurent series
$$
...
**Solution.** $z^3 - z^4 = z^3(1 - z)$ shows that $f(z)$ is singular at $z = 0$ and $z = 1$. Now $z = 1$ lies outside $C$. Hence it is of no interest here. So we need the residue of $f(z)$ at 0. We find it from the Laurent series that converges for $0 < |z| < 1$. This is series (I) in Example 4, @sec-laurent-series-sec,
$$
...
**CAUTION!** Had we used the wrong series (II) in Example 4, @sec-laurent-series-sec,
$$
...
In @eq-16-3-4 we assume that $f(z) = p(z)/q(z)$ with $p(z_0) \ne 0$ and $q(z)$ has a simple zero at $z_0$, so that $f(z)$ has a simple pole at $z_0$ by Theorem 4 in @sec-singularities-zeros.

**PROOF** We prove @eq-16-3-3. For a simple pole at $z_0$ the Laurent series (1), @sec-laurent-series-sec, is
$$
...
$$
where the last equality follows from continuity (Theorem 1, <!-- TODO: replace Sec. 15.3 with actual cross-reference label -->).
...
**PROOF** We prove @eq-16-3-5. The Laurent series of $f(z)$ converging near $z_0$ (except at $z_0$ itself) is (@sec-singularities-zeros)
$$
...
$$
We see that $b_1$ is now the coefficient of the $(m-1)$th power $(z - z_0)^{m-1}$ of the power series of $g(z) = (z - z_0)^m f(z)$. Hence Taylor's theorem (<!-- TODO: replace Sec. 15.4 with actual cross-reference label -->) gives @eq-16-3-5:
$$
...
![Fig. 373. Residue theorem](../images/chapter16/fig-16-373.png){#fig-16-373}
...
the integral along $C$ being taken counterclockwise and the other integrals clockwise (as in @fig-14-16 and @fig-14-17, in @sec-cauchy-integral-theorem). We take the integrals over $C_1, \ldots, C_k$ to the right and compensate the resulting minus sign by reversing the sense of integration. Thus,
...
## Residue Integration of Real Integrals {#sec-16-4}
...
Since $F$ is rational in $\cos\theta$ and $\sin\theta$, @eq-16-4-2 shows that $F$ is now a rational function of $z$, say, $f(z)$. Since $d\theta = dz/(iz)$, we have $J = \oint_C f(z)\,dz/iz$, and, as $\theta$ ranges from 0 to $2\pi$ in @eq-16-4-1, the variable $z$ ranges counterclockwise once around the unit circle $C$. (Review <!-- TODO: replace Sec. 13.5 with actual cross-reference label --> if necessary.)
...
![Fig. 374. Path $C$ of the contour integral in (5*)](../images/chapter16/fig-16-374.png){#fig-16-374}
...
for sufficiently large constants $k$ and $R_0$. By the ML-inequality in <!-- TODO: replace Sec. 14.1 with actual cross-reference label -->,
...
![Fig. 375. Example 2](../images/chapter16/fig-16-375.png){#fig-16-375}
...
(Here we used $e^{i\pi} = -1$ and $e^{-2\pi i} = 1$.) By (1) in <!-- TODO: replace Sec. 13.6 with actual cross-reference label --> and @eq-16-4-7 in this section,
$$
...
as they occur in connection with the Fourier integral (<!-- TODO: replace Sec. 11.7 with actual cross-reference label -->).
...
Since $e^{isx} = \cos sx + i\sin sx$, this yields the above results [see also (15) in <!-- TODO: replace Sec. 11.7 with actual cross-reference label -->.] $\blacksquare$
...
![Fig. 376. Theorem 1](../images/chapter16/fig-16-376.png){#fig-16-376}

**PROOF** By the definition of a simple pole (@sec-singularities-zeros) the integrand has for $0 < |z - a| < R$ the Laurent series
$$
...
$$
The second integral on the right cannot exceed $M\pi r$ in absolute value, by the ML-inequality (<!-- TODO: replace Sec. 14.1 with actual cross-reference label -->), and $M\pi r \to 0$ as $r \to 0$. $\blacksquare$

![Fig. 377. Application of Theorem 1](../images/chapter16/fig-16-377.png){#fig-16-377}
...
$$
(This integral is needed in heat conduction in <!-- TODO: replace Sec. 12.7 with actual cross-reference label -->.)

(c) **Inspection.** Solve Probs. 13 and 17 without calculation.


## @ch-laurent-series-residue-integration Review Questions and Problems {#sec-16-review}
...
## Summary of @ch-laurent-series-residue-integration {#sec-16-summary}
...
If the principal part is an infinite series, the singularity of $f(z)$ at $z_0$ is called an **essential singularity** (@sec-singularities-zeros).

@sec-singularities-zeros also discusses the **extended complex plane**, that is, the complex plane with an improper point ("infinity") attached.
