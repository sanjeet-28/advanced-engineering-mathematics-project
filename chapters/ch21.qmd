...
discussed in Sec. 1.2. Geometrically, this is an approximation of the curve of $y(x)$ by a polygon whose first side is tangent to this curve at $x_0$ (see <!-- TODO: replace Fig. 8 in Sec. 1.2 with actual cross-reference label -->Fig. 8 in Sec. 1.2).
...
**Table: Improved Euler Method (Heun’s Method)** {#tbl-improved-euler-heun}
...
**Table: Improved Euler Method for (@eq-sec211-6). Errors** {#tbl-improved-euler-errors}
...
**Table: Classical Runge–Kutta Method of Fourth Order** {#tbl-classical-runge-kutta}
...
**Table: Runge–Kutta Method Applied to (@eq-sec211-6)** {#tbl-rk-applied}
...
**Table: Comparison of the Accuracy of the Three Methods under Consideration in the Case of the Initial Value Problem (@eq-sec211-6), with $h = 0.2$** {#tbl-accuracy-comparison}
...
**@tbl-rk-applied-error-estimate: Runge–Kutta Method Applied to the Initial Value Problem (@eq-sec211-11) and Error Estimate (@eq-sec211-10). Exact Solution $y = \tan x + x + 1$**
...
**Table: Methods Considered and Their Order (= Their Global Error)** {#tbl-methods-order}
...
**Table: Backward Euler Method (BEM) for Example 4. Comparison with Euler and RK** {#tbl-backward-euler}
...
**Table: Adams–Moulton Method Applied to the Initial Value Problem (@eq-sec212-8); Predicted Values Computed by (@eq-sec212-7a) and Corrected Values by (@eq-sec212-7b)** {#tbl-adams-moulton}
...
**Table: Euler Method for Systems in Example 1 (Mass–Spring System)** {#tbl-euler-systems}
...
**Table: RK Method for Systems: Values $y_{1,n}(x_n)$ of the Airy Function $	ext{Ai}(x)$ in Example 2** {#tbl-rk-systems}
...
**Table: Runge–Kutta–Nyström Method Applied to Airy’s Equation, Computation of the Airy Function $y = 	ext{Ai}(x)$** {#tbl-rkn-applied}
...
**Table: Backward Euler Method (BEM) for Example 4. Comparison with Euler and RK** {#tbl-backward-euler-systems}
...
@fig-21-02 shows the Euler method for $h = 0.18$, an interesting case with initial jumping (for about $x > 3$) but later monotone following the solution curve of $y = y_1$. See also CAS Experiment 15. $\blacksquare$
...
**Solution.**  
We use the grid shown in @fig-21-08b, where $h = 0.5$. We recall that (7) in @sec-methods-elliptic-pdes has the right side $h^2f(x, y) = 0.5^2 \cdot 12xy = 3xy$. From the formulas $u = 3y^3$ and $u_n = 6x$ given on the boundary we compute the boundary data:
...
For instance, (@eq-sec217-1)–(@eq-sec217-4) is the model of a vibrating elastic string with fixed ends at $x = 0$ and $x = 1$ (see Sec. 12.2). Although an analytic solution of the problem is given in <!-- TODO: verify equation reference -->(13), Sec. 12.4, we use the problem for explaining basic ideas of the numeric approach that are also relevant for more complicated hyperbolic PDEs.
...
The reason for the exactness follows from d'Alembert's solution <!-- TODO: verify equation reference -->(4), Sec. 12.4. (See Prob. 10, below.) $\blacksquare$
...
4. **Another starting formula.** Show that <!-- TODO: verify equation reference -->(12) in Sec. 12.4 gives the starting formula
...
7. **Zero initial displacement.** If the string governed by the wave equation (1) starts from its equilibrium position with initial velocity $g(x) = \sin \pi x$, what is its displacement at time $t = 0.4$ and $x = 0.2, 0.4, 0.6, 0.8$? (Use the present method with $h = 0.2, k = 0.2$. Use (@eq-sec217-8). Compare with the exact values obtained from <!-- TODO: verify equation reference -->(12) in Sec. 12.4.)
...
10. Show that from d’Alembert’s solution <!-- TODO: verify equation reference -->(13) in Sec. 12.4 with $c = 1$ it follows that (@eq-sec217-6) in the present section gives the exact value $u_{i, j+1} = u(ih, (j + 1)h)$.
