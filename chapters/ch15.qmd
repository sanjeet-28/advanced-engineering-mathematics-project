---
title: "CHAPTER 15: Power Series, Taylor Series"
---
...
## 15.1 Sequences, Series, Convergence Tests {#sec-sequences-series-tests}
...
![Fig. 361. Convergent complex sequence](../images/fig-15-01.png){#fig-15-01}

![Fig. 362. Convergent real sequence](../images/fig-15-02.png){#fig-15-02}
...
**PROOF** Convergence $z_n 	o c = a + ib$ implies convergence $x_n 	o a$ and $y_n 	o b$ because if $|z_n - c| < \epsilon$, then $z_n$ lies within the circle of radius $\epsilon$ about $c = a + ib$, so that (@fig-15-03a)
$$
...
$$
These two inequalities imply that $z_n = x_n + i y_n$ lies in a square with center $c$ and side $\epsilon$. Hence, $z_n$ must lie within a circle of radius $\epsilon$ with center $c$ (@fig-15-03b). $\square$

![Fig. 363. Proof of Theorem 1](../images/fig-15-03.png){#fig-15-03}
...
![Fig. 364. Convergence of the series $1 + 1/4 + 1/9 + 1/16 + \dots$](../images/fig-15-04.png){#fig-15-04}
...
## 15.2 Power Series {#sec-power-series}
...
$$
converges absolutely if $|z| < 1$ and diverges if $|z| \ge 1$ (see Theorem 6 in Sec. 15.1). $\square$
...
![Fig. 365. Theorem 1 convergence and divergence regions](../images/fig-15-05.png){#fig-15-05}
...
ight| < 1$. Hence the series on the right side of @eq-15-2-3 is a converging geometric series (see Theorem 6 in Sec. 15.1). Absolute convergence of @eq-15-2-1 as stated in (b) now follows by the comparison test in Sec. 15.1.  
...
![Fig. 366. Circle of convergence](../images/fig-15-06.png){#fig-15-06}
...
**PROOF** For @eq-15-2-1 the ratio of the terms in the ratio test (Sec. 15.1) is
$$
...
eq z_0$ by the ratio test (Theorem 7, Sec. 15.1). $\square$
...
## 15.3 Functions Given by Power Series {#sec-functions-given-by-power-series}
...
**PROOF** This follows from @eq-15-2-6 in Sec. 15.2 because
$$
...
$$ {#eq-15-3-7}
This series is the second derived series of $\sum a_n z^n$ at $z = R_0$ and converges absolutely by Theorem 3 of this section and Theorem 1 of Sec. 15.2. Hence our present series @eq-15-3-7 converges. Let the sum of @eq-15-3-7 (without the factor $|\Delta z|$) be $K(R_0)$. Since $K(R_0)$ is a constant, our present result is:
$$
...
## 15.4 Taylor and Maclaurin Series {#sec-taylor-maclaurin-series}
...
$$ {#eq-15-4-1}
or, by Cauchy's integral formula for derivatives (Sec. 14.4),
$$
...
![Fig. 367. Contours for Cauchy formula in Taylor's theorem](../images/fig-15-07.png){#fig-15-07}
...
ight| \le 	ilde{M}
$$
for all $z^*$ on $C$. Also, $C$ has the radius $r = |z^* - z_0|$ and the length $2\pi r$. Hence by the ML-inequality (Sec. 14.1) we obtain from @eq-15-4-3
$$
...
We know that the exponential function $e^z$ (Sec. 13.5) is analytic for all $z$, and $(e^z)' = e^z$. Hence from @eq-15-4-1 with $z_0 = 0$ we obtain the Maclaurin series
$$
...
Furthermore, by setting $z = iy$ in @eq-15-4-12 and separating the series into the real and imaginary parts (see Theorem 2, Sec. 15.1) we obtain
$$
...
By substituting @eq-15-4-12 into (1) of Sec. 13.6 we obtain
$$
...
$$
When $z = x$ these are the familiar Maclaurin series of the real functions $\cos x$ and $\sin x$. Similarly, by substituting @eq-15-4-12 into (11), Sec. 13.6, we obtain
$$
...
### EXAMPLE 4 Logarithm
From @eq-15-4-11 it follows that by integration (Theorem 4, Sec. 15.3) we obtain
$$
...
## 15.5 Uniform Convergence. Optional {#sec-uniform-convergence}
...
**Solution.** (a) For $z$ in that closed disk we have $|1 - z| \ge 1 - r$ (sketch it). This implies that $1 / |1 - z| \le 1 / (1 - r)$. Hence (remember @eq-15-4-8 in Sec. 15.4 with $q = z$)
$$
...
$$ {#eq-15-5-3}
Now @eq-15-5-2 converges absolutely if $|z - z_0| = r < R$ (by Theorem 1 in Sec. 15.2). Hence it follows from the Cauchy convergence principle (Sec. 15.1) that, an $\epsilon > 0$ being given, we can find an $N(\epsilon)$ such that
$$
...
![Fig. 368. Partial sums in Example 2](../images/fig-15-08.png){#fig-15-08}
...
$$
Let $L$ be the length of $C$. Since the given series converges uniformly, for every given $\epsilon > 0$ we can find a number $N$ such that $|R_n(z)| < \epsilon / L$ for all $n \ge N$ and all $z$ in $G$. By applying the ML-inequality (Sec. 14.1) we thus obtain
$$
...
1. **CAS EXPERIMENT. Graphs of Partial Sums.**
   (a) **Fig. 368.** Produce this exciting figure (cf. @fig-15-08) using your CAS. Add further curves, say, those of $s_{256}$, $s_{1024}$, etc. on the same screen.
...
18. **TEAM PROJECT. Uniform Convergence.**  
    ...
    (e) **Figure 369.** Show that $x^2 \sum_{m=1}^{\infty} (1 + x^2)^{-m} = 1$ if $x \neq 0$ and $0$ if $x = 0$. Verify by computation that the partial sums $s_1, s_2, s_3$ look as shown in Fig. 369.

![Fig. 369. Sum $s$ and partial sums in Team Project 18(e)](../images/fig-15-09.png){#fig-15-09}

### 19–20 HEAT EQUATION
Show that (9) in Sec. 12.6 with coefficients (10) is a solution of the heat equation for $t > 0$, assuming that $f(x)$ is continuous on the interval $0 \le x \le L$ and has one-sided derivatives at all interior points of that interval. Proceed as follows.
